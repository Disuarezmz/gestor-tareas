...
function KanbanColumns({ data = KANBAN_DATA, dense, columnWidth }) {
  return (
...
function KanbanA() {
  // Clásico — 4 columnas, project context bar
  return (
    <Page>
...
function KanbanB() {
  // Swim-lanes — filas por proyecto, columnas por estado
  const lanes = [
...
function KanbanC() {
  // Vertical compact — columnas estrechas tipo Twitter, denso
  return (
    <Page>
      <TopBar active="kanban" />
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
...
function KanbanD() {
  // Bento mosaico — tamaños distintos según prioridad (atrevido)
  const items = {
    new: [
      ['Refactor módulo auth', 'high', 'L'],
      ['Bug: select se cierra', 'high', 'M'],
      ['Comprar dominio', 'low', 'S'],
      ['Investigar libs gráficos', 'med', 'S'],
    ],
    wait: [
      ['Aprobación legal', 'high', 'L'],
      ['Llamada diseño', 'med', 'S'],
    ],
    exec: [
      ['Onboarding rediseño', 'high', 'XL'],
      ['Migrar a Postgres 16', 'high', 'L'],
      ['Revisar PR #482', 'med', 'M'],
      ['Cabecera landing', 'med', 'S'],
      ['Test e2e checkout', 'med', 'S'],
    ],
    done: [
      ['Setup CI', 'med', 'S'],
      ['Sketch v1', 'high', 'M'],
...
Object.assign(window, { KanbanA, KanbanB, KanbanC, KanbanD });
