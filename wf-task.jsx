// Task detail + create/edit variations

// ── DETALLE ────────────────────────────────────────────────────────────

function DetailA() {
  // Panel lateral slide-in (sobre el kanban)
  return (
    <Page>
          ...
          <div style={{ flex: 1, overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 280px', gap: 0 }}>
            <div style={{ padding: '14px 22px 22px', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <SB style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <Mono>SUBTAREAS</Mono>
                  <Mono color={wfTokens.text}>3 / 5</Mono>
                  <div style={{ flex: 1 }} />
                  <Mono>+ añadir</Mono>
                </div>
                {[
                  ...
                  ['Animaciones de transición', false],
              ...
              <SB style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                {[
                  ['tú','hace 2h','He cerrado los estados de error, falta animar las transiciones.'],
                  ['diseño','ayer','Revisado, ojo con el contraste del paso 2.'],
                ].map(([w, when, txt], i) => (
    ...
    <Page>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <SB style={{
            width: 620, maxHeight: '90%', padding: 0,
            background: wfTokens.surface, boxShadow: '0 20px 60px rgba(0,0,0,0.7)',
            display: 'flex', flexDirection: 'column',
          }}>
            {/* Modal header */}
            <div style={{ padding: '14px 18px', borderBottom: `1px solid ${wfTokens.borderSoft}`, display: 'flex', alignItems: 'center', gap: 10 }}>
              <Check size={14} />
              <div style={{ flex: 1, fontFamily: '"Caveat", cursive', fontSize: 24, color: wfTokens.text }}>
                Cerrar el diseño del onboarding
              </div>
              <Ic d={I.link} size={12} />
              <Ic d={I.more} size={12} />
              <Ic d={I.x} size={14} />
                  ...
                  <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {[1,2].map(i => (
                      <div key={i} style={{ display: 'flex', gap: 6 }}>
                        <div style={{ width: 18, height: 18, borderRadius: 999, background: wfTokens.surfaceHi, flexShrink: 0 }} />
                        <Lines widths={[90,60]} h={4} c={wfTokens.borderSoft} />
                      </div>
                    ))}
                  </div>
                </div>
  ...
  return (
    <Page>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <SB style={{
            width: 540, padding: 0, background: wfTokens.surface,
            ...
            <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <Field label="Proyecto">
                  <Select value={<><Dot c={wfTokens.hueExec} /> Rediseño app</>} />
                </Field>
                <Field label="Estado">
                  <Select value={<><StateDot k="new" /> Nueva</>} />
                </Field>
                <Field label="Fecha límite">
                  <Select value={<><Ic d={I.cal} size={10} /> 27 mayo · 18:00</>} />
                </Field>
              ...
              <Field label="Recordatorio">
                <div style={{ display: 'flex', gap: 6 }}>
                  {['10m','1h','1d','custom'].map((r,i) => (
                    <div key={r} style={{ padding: '5px 10px', borderRadius: 4, border: `1px solid ${i===1?'var(--wf-accent)':wfTokens.border}`, fontSize: 10, color: i===1?wfTokens.text:wfTokens.textMuted, background: i===1?`color-mix(in oklch, var(--wf-accent) 15%, transparent)`:'transparent' }}>{r}</div>
                  ))}
                </div>
              </Field>
            </div>
...
function CreateB() {
  // Command bar rápido — lenguaje natural
  return (
    <Page>
      <TopBar active="kanban" />
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', justifyContent: 'center', paddingTop: 100 }}>
          <div style={{ width: 600, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <SB style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 4 }}>
              <Mono>ATAJOS</Mono>
              <div style={{ display: 'grid', gridTemplateColumns: '50px 1fr', gap: 4, marginTop: 4 }}>
                {[['@','proyecto'],['#','etiqueta'],['!','prioridad (baja/media/alta)'],['tiempo','hoy, mañana, vie, 27/05, 18h, +2d'],['/','asignar a estado: /espera, /ejec, /done']].map(([k,v],i)=>(
                  <div key={i} style={{ display: 'contents' }}>
                    <Mono color={wfTokens.text}>{k}</Mono>
                    <Mono>{v}</Mono>
                  </div>
                ))}
              </div>
...
function CreateC() {
  // Página split: formulario + preview de la card
  return (
    <Page>
      <TopBar active="kanban" />
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 320px', gap: 0 }}>
            <div style={{ padding: '14px 22px 22px', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <Field label="Proyecto"><Select value={<><Dot c={wfTokens.hueExec}/> Rediseño app</>} /></Field>
                <Field label="Estado"><Select value={<><StateDot k="new"/> Nueva</>} /></Field>
                <Field label="Prioridad"><Select value={<Prio level="high"/>} /></Field>
                <Field label="Fecha"><Select value={<><Ic d={I.cal} size={10}/> 27 may</>} /></Field>
                <Field label="Hora"><Select value={<Mono color={wfTokens.text}>18:00</Mono>} /></Field>
                <Field label="Recordatorio"><Select value={<Mono color={wfTokens.text}>1h antes</Mono>} /></Field>
              </div>
            </div>
            {/* Live preview */}
            <div style={{ borderLeft: `1px solid ${wfTokens.borderSoft}`, background: wfTokens.surfaceLo, padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <TaskCard
                title="Cerrar diseño del onboarding y exportar specs"
                prio="high"
                due="27 may"
                tags={['ux','ios']}
                subs={[0,4]}
                state="new"
              />
              <Mono>VISTA PREVIA EN LISTA</Mono>
              <SB hi style={{ padding: 8, display: 'flex', alignItems: 'center', gap: 8, borderLeft: `2px solid ${stateColor('new')}` }}>
                <Check size={11} />
                <span style={{ fontSize: 11, flex: 1, color: wfTokens.text }}>Cerrar diseño...</span>
                <Prio level="high" />
              </SB>
              <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
                <Btn primary>crear tarea</Btn>
...
function DetailContent() {
  // Content used inside slide-in panel A
  return (
    <>
...
Object.assign(window, { DetailA, DetailB, DetailC, CreateA, CreateB, CreateC });
