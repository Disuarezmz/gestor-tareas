...
import { Page, Mono, SB, L, Lines, Dot, StateDot, StatePill, Btn, Ic, HW } from '../../primitives/index.jsx';
...
export function CalendarA() {
  const days = Array.from({ length: 35 }, (_, i) => i - 3);
  const tasksOnDay = {
    1: [['Standup', 'exec']], 3: [['Demo', 'wait']], 7: [['Sprint plan', 'exec']],
    14: [['Sketch v1', 'done']], 17: [['Cert SSL', 'done']], 20: [['Kick-off Q3', 'done']],
    22: [['Sketch v1 cierre', 'done']], 24: [['Setup CI', 'done']],
    27: [['Onboarding', 'exec'], ['Revisar PR', 'exec'], ['Llamada', 'wait']],
    28: [['Cabecera landing', 'exec']], 29: [['E2E checkout', 'new']],
    30: [['Legal copy', 'wait']], 31: [['Demo Acme', 'new']],
  };
  return (
    <Page>
...
export function CalendarB() {
  const days = [['lun', '25'], ['mar', '26'], ['mié', '27', true], ['jue', '28'], ['vie', '29'], ['sáb', '30'], ['dom', '31']];
  const hours = ['08', '10', '12', '14', '16', '18', '20'];
  const events = [
    [0, 10, 18, 'Standup', 'exec'], [0, 40, 25, 'Diseño sprint', 'exec'],
    [1, 30, 30, 'Sprint plan', 'exec'], [2, 5, 12, 'Standup', 'exec'],
    [2, 30, 35, 'Onboarding rediseño', 'exec'], [2, 70, 15, 'Llamada diseño', 'wait'],
    [3, 15, 30, 'Cabecera landing', 'exec'], [3, 60, 20, 'Review PR', 'exec'],
    [4, 5, 15, 'Standup', 'exec'], [4, 30, 40, 'E2E checkout', 'new'],
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '40px repeat(7, 1fr)', borderTop: `1px solid ${wfTokens.borderSoft}`, borderLeft: `1px solid ${wfTokens.borderSoft}` }}>
              ...
              {days.map(([d, n, today], i) => (
                <div key={i} style={{ padding: '6px 8px', borderRight: `1px solid ${wfTokens.borderSoft}`, borderBottom: `1px solid ${wfTokens.borderSoft}`, background: today ? `color-mix(in oklch, var(--wf-accent) 10%, transparent)` : 'transparent' }}>
                  <Mono size={9} color={wfTokens.textDim}>{d.toUpperCase()}</Mono>
                  <div style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: 16, marginTop: 2, color: today ? wfTokens.text : wfTokens.textMuted }}>{n}</div>
                </div>
              ))}
            </div>
              ...
              return (
                <SB key={m} style={{ padding: 10 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
                    {['l', 'm', 'x', 'j', 'v', 's', 'd'].map((d) => <Mono key={d} size={7} style={{ textAlign: 'center' }}>{d}</Mono>)}
          ...
          <div style={{ padding: '0 22px 22px', flex: 1, overflow: 'hidden', display: 'grid', gridTemplateColumns: '220px 1fr 280px', gap: 14 }}>
            ...
            <SB style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: 12, padding: '10px 14px', borderBottom: i < 5 ? `1px solid ${wfTokens.borderSoft}` : 'none' }}>
